...
void ofApp::draw() {
	...
	for (int r = 60; r > 10; r -= 5) {

		for (int v = 0; v < 360; v += v_span) {

			auto u_start = ofMap(ofNoise(glm::vec3(R * cos(v * DEG_TO_RAD) * 0.01, R * sin(v * DEG_TO_RAD) * 0.01, r * 0.01 + ofGetFrameNum() * 0.0075)), 0, 1, -360, 360);
			for (int u = u_start; u < u_start + 180; u += u_span) {
				...
				ofColor face_color = ofColor(ofMap(r, 10, 60, 255, 0));
